...
{
    $prevStart = get_post_meta($post->ID, 'asgssp_timespan_start', true);
    $prevEnd = get_post_meta($post->ID, 'asgssp_timespan_end', true);
...
function asgssn_metabox_timespan_save($post_id)
{
        ...
        update_post_meta($post_id, 'asgssp_timespan_start', $newStart);
        update_post_meta($post_id, 'asgssp_timespan_end', $newEnd);
}
